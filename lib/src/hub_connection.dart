...
class HubConnection {
  ...
  Stream<Object?> stream(String methodName, List<Object> args) {
    ...
    final StreamController streamController = StreamController<Object?>(
      onCancel: () {
        ...
        final formattedCancelMessage = _protocol.writeMessage(cancelMessage);
        _callbacks.remove(invocationMessage.invocationId);
        _sendMessage(formattedCancelMessage);
      },
    ...
    final formattedMessage = _protocol.writeMessage(invocationMessage);
    _sendMessage(formattedMessage).catchError((dynamic error) {
  ...
  void _processIncomingData(Object? data) {
    ...
    _logger?.finest("Incoming message");
